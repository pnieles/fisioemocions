...
function UsuariosPage() {
  ...
  const create = useMutation({
    mutationFn: async () => {
      if (!form.email || form.password.length < 6)
        throw new Error("Correo válido y contraseña de mínimo 6 caracteres");
  ...
  const changeRole = useMutation({
    mutationFn: async (v: { user_id: string; role_id: string }) => {
      await updRole({ data: v });
    },
    onSuccess: () => {
        ...
        <CardContent className="p-6 space-y-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Crea una cuenta con correo, contraseña y rol.
            </p>
          </div>
        ...
        <CardContent className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            {users.map((u) => (
              <div
                key={u.user_id}
                className="border border-border rounded-md p-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-end"
              >
                ...
                <div className="md:col-span-2 flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    onClick={() => {
                      if (confirm(`¿Eliminar ${u.email}?`)) remove.mutate(u.user_id);
                    }}
