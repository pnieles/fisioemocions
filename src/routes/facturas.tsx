...
function FacturasPage() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 max-w-[1400px] mx-auto">
      <PageHeader
        title="Facturas"
        subtitle="Facturas CASS (auto-numeradas) y Privado (exportables en PDF)."
      />
          ...
          <div className="px-6 py-4 border-b border-border flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Marca el estado para hacer seguimiento del cobro.
              </p>
            </div>
                  ...
                  return (
                    <tr key={i.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      <td className="px-4 py-2 text-muted-foreground text-xs">
                        {new Date(i.issue_date).toLocaleString("es-ES", {
                          dateStyle: "short",
                          timeStyle: "short",
                        })}
                      </td>
                      ...
                      <td className="px-4 py-2 text-right tabular-nums">
                        {eur(Number(i.base_amount))}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums text-muted-foreground">
                        {eur(Number(i.igi_amount))}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums font-medium">
                        {eur(Number(i.total_amount))}
                      </td>
                      <td className="px-4 py-2">
                        <Select
                          value={i.status}
                        ...
                        {i.patient_type === "privado" && (
                          <Button
                            ...
                            onClick={() =>
                              exportInvoicePdf(
                                i,
                                company ?? { name: "fisioemocions", logo_url: null },
                                p,
                              )
                            }
                          >
